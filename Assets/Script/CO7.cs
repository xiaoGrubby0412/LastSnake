...
public class CO7 : MonoBehaviour {
	...
	public void OnTriggerEnter(Collider collider){
		...
		if (sssss == "node"||nodeFod == "AI1foo" || nodeFod == "AI2foo" || nodeFod == "AI3foo" || nodeFod == "AI4foo" || nodeFod == "AI5foo" || nodeFod == "AI6foo" || nodeFod == "AI7foo" || nodeFod == "AI8foo" || nodeFod == "AI9foo" || nodeFod == "AI10fo" || nodeFod == "mefood"||nodeFod=="AI11fo") {
			int aic = AIControl.pos[6,0].SnakeLength;
			if(aic>36){
				Destroy (collider.gameObject);//删除吃到的食物对象
				AIControl.pos [6, 0].AISnakeCount=0;//记录数变成6
				return;
			}
			GameObject ite;
			AIControl.pos [6, 0].AISnakeCount++;
			int bianliang = AIControl.pos [6, 0].AISnakeCount;
			if (bianliang % 6 == 0) {
				for(int inter=1;inter<=3;inter++)
				{
				AIControl.pos [6, 0].SnakeLength++;
					if(inter%3==0)
				 ite = (GameObject)Instantiate (Resources.Load ("body18"));
					else
						ite = (GameObject)Instantiate (Resources.Load ("body8_1"));
				GameObject father = GameObject.Find ("AI7");
				...
				ite.name = "AIseven" + AIControl.pos [6, 0].SnakeLength.ToString();
				ite.transform.localPosition = new Vector3 (8888, 8888, 0);
			//	mingci.SLength7 = AIControl.pos [6, 0].SnakeLength;
				mingci.St[6].length = AIControl.pos[6,0].SnakeLength;
			}
			}
			Destroy (collider.gameObject);//删除吃到的食物对象
		}
