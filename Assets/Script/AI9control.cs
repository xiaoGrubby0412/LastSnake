...
public class AI9control : MonoBehaviour {

	GameObject obj;
	// Use this for initialization
	void Start () {
		obj= GameObject.Find("AI9Panduan");
	}
	...
	public void AInine(){
		...
		//if (bleed == false)
		//return;
		if (AIControl.AISnakeCanMove [8] == false)//第二条蛇是否死亡
			return;
		Vector3 v = new Vector3 (AIControl.pos [8, 0].SnakePos.x, AIControl.pos [8, 0].SnakePos.y, 0);
		//print (v.x + "," + v.y);
		int sp = AIControl.pos [8, 0].SnakeSpeed;
		//	print ("速度是" + sp);
	//	int length = AIControl.pos [8, 0].SnakeLength;
		//print ("长度是" + length);
	//	string name = AIControl.pos [8, 0].whoSnake;
		//print ("名字是" + name);
		//GameObject obj = GameObject.Find (name);
		//print("线程进了这个方法");
		obj.transform.localPosition += sp * v.normalized;//给蛇产生位置
		//print("fff");
		GameObject dd = GameObject.Find ("AISnakeHead9");
		ai9.AIXuaanzhuang (v.x, v.y,dd);
		//Vector3 yy = obj1.transform.localPosition;
		flootAIOneBody9 (obj.transform.localPosition);//此函智能在最下面   不然 他下面的函数不能执行
		
	}
	//private GameObject SnakeHead;
	private Vector3 headF = Vector3.zero, headL,bodyF =new Vector3(0,0,0),v3 = Vector3.zero,v1 = Vector3.zero;
	public void flootAIOneBody9(Vector3 V){//v移动前坐标 c1 蛇头名字 s1 找到要添加的AI1
		Vector3 v = new Vector3 (V.x, V.y, 0);
		//print (v.x + "," + v.y);
		//SnakeHead=GameObject.Find("AI9Panduan");
		GameObject Map = GameObject.Find("AI9");
		...
		//print ("程序执行到这里"+(c - 2));
		for (int i=1; i<c; i++) {
			GameObject body = GameObject.Find("AInine"+i.ToString());
