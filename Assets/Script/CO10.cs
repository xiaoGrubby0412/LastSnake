...
public class CO10 : MonoBehaviour
{
    ...
    public void OnTriggerEnter(Collider collider)
    {
        ...
        if (sssss == "node" || nodeFod == "AI1foo" || nodeFod == "AI2foo" || nodeFod == "AI3foo" ||
            ...
            nodeFod == "AI11fo")
        {
            int aic = AIControl.pos[9, 0].SnakeLength;
            if (aic > 36)
            {
                Destroy(collider.gameObject); //删除吃到的食物对象
                AIControl.pos[9, 0].AISnakeCount = 0; //记录数变成6
                return;
            }

            GameObject ite;
            AIControl.pos[9, 0].AISnakeCount++;
            int bianliang = AIControl.pos[9, 0].AISnakeCount;
            if (bianliang % 6 == 0)
            {
                for (int inter = 1; inter <= 3; inter++)
                {
                    AIControl.pos[9, 0].SnakeLength++;
                    if (inter % 3 == 0)
                        ite = (GameObject)Instantiate(Resources.Load("body4"));
                    else
                        ite = (GameObject)Instantiate(Resources.Load("body8_1"));
                    GameObject father = GameObject.Find("AI10");
                    ...
                    ite.name = "AIten" + AIControl.pos[9, 0].SnakeLength.ToString();
                    ite.transform.localPosition = new Vector3(8888, 8888, 0);
                    //	mingci.SLength10 = AIControl.pos [9, 0].SnakeLength;
                    mingci.St[9].length = AIControl.pos[9, 0].SnakeLength;
                }
            }
