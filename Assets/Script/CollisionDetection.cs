...
//	AIBB aibb = new AIBB();
    ...
    public void OnTriggerEnter(Collider collider)
    {
        ...
        else
        {
            ...
            if (sssss == "node")
            {
                ...
                if (SnakeCC % 3 == 0)
                {
                    for (int inter = 1; inter <= 3; inter++)
                    {
                        ...
                        if (SnakeControl.SnakeCout >= 100 && SnakeControl.SnakeCout < 200)
                        {
                            //如果蛇吃到了一定长度的食物就让他的大小增加一定的范围
                        ...
                        else if (SnakeControl.SnakeCout >= 200)
                        {
                            //如果长度大于了200
                            ...
                            if (IsSizeTwo == true)
                            {
                                //需要主要的是在单机返回和重新开始的时候需要把这两个bool变成false
                                //蛇头也要初始化为默认的大小
                                IsSizeTwo = false;
                                for (int i = 1; i < SnakeControl.SnakeCout; i++)
                                {
            ...
            if (nodeFod == "AI1foo" || nodeFod == "AI2foo" || nodeFod == "AI3foo" || nodeFod == "AI4foo" ||
                nodeFod == "AI5foo" || nodeFod == "AI6foo" || nodeFod == "AI7foo" || nodeFod == "AI8foo" ||
                nodeFod == "AI9foo" || nodeFod == "AI10fo" || nodeFod == "mefood" || nodeFod == "AI11fo")
            {
                GameObject ite;
                againFood++;
                Maxfood++;
